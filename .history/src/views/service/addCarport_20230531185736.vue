	...
	<div class="box">
		...
		<div class="vant-form">
			<van-cell-group>
				...
				<van-dropdown-menu v-model="form.parkId" label="小区名字">
					<van-dropdown-item v-for="item in communityList" :key="item.id" :value="item.parkName">
						{{ item.parkName }}
					</van-dropdown-item>
				</van-dropdown-menu>
				<van-popup v-model="form.parkId" round position="bottom">
					<van-picker v-for="item in communityList" :columns="item.id" :value="item.parkName" show-toolbar title="选择选项" @change="handleChange" />
				</van-popup>
