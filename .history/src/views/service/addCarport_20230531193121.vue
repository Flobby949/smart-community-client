	...
	<div class="box">
		...
		<div class="vant-form">
			<van-cell-group>
				...
				<van-field v-model="form.communityId" label="小区名称" placeholder="请选择小区名称" readonly @click="showPicker" />
...
const showPicker = () => {
	Picker({
		...
		columns: [
			{
				values: communityList.value.map(item => item.communityName),
				defaultIndex: 0
			}
		],
		onSelect: ([selectedValue]) => {
			const selectedCommunity = communityList.value.find(item => item.communityName === selectedValue)
