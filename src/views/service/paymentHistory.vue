			...
			<div v-for="(item, index) in bills" :key="index" class="rounded-lg shadow-md p-4 relative mb-6 bg-white">
				<div class="mt-0 p-3">
					...
					<div class="pt-5">
						...
						<div class="flex relative">
							<p class="text-gray-500 pt-1">账单金额</p>
							<div class="absolute right-2 p-1 top-[-1px]">￥{{ item.orderMoney }}元</div>
						</div>
						<div class="flex relative">
							<p class="text-gray-500 pt-1">缴费时间</p>
							<div class="absolute right-2 p-1 top-[-1px]">{{ item.payTime }}</div>
						</div>
